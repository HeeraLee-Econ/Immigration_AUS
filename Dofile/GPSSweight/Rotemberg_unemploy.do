...
*          Outcome: unemployment rate (unempshare_ageall)
**********************************************************************
clear all
...
**********************************************************************
capture log close
log using "$output/Log/Rotemberg_unemploy.smcl", replace
...
**********************************************************************
* 3. 표본/컨트롤 설정 (ABS_robust.do와 동일한 baseline lag control, 2001-2021 표본)
**********************************************************************
...
*   FWL을 직접 적용)
**********************************************************************
xi: reg unempshare_ageall $demo year_1-year_5 i.LGAFINAL21, cluster(LGAFINAL21)
predict yhat, xb
gen p_t_res = unempshare_ageall - yhat
...
save "$interim/ABS/Rotemberg/Immi_res_unemploy.dta", replace
...
**********************************************************************
use "$interim/ABS/Rotemberg/Immi_res_unemploy.dta", clear
...
save "$interim/ABS/Rotemberg/Rotemberg_unempl_results.dta", replace
...
use "$interim/ABS/Rotemberg/Rotemberg_unempl_results.dta", clear
merge 1:1 con_id using `vz', nogen
save "$interim/ABS/Rotemberg/Rotemberg_unempl_results.dta", replace

* 전체 29개국 결과 CSV로 바로 export (Panel C에서 top5만 자르기 전, 국가 전체를 눈으로 보고 싶을 때 사용)
export delimited using "$output/Table/RotembergAll_unemploy.csv", replace
...
foreach stat of local cutstats {
    foreach zzz of local zzzopts {
        ...
        graph export "$output/Figure/fig_rotemberg_bubble_unemploy_`stat'cut_`zzz'ZZZ.pdf", replace
        graph export "$output/Figure/fig_rotemberg_bubble_unemploy_`stat'cut_`zzz'ZZZ.png", replace
...
export delimited using "$output/Table/RotembergA_negpos_unemploy.csv", replace
restore
...
export delimited using "$output/Table/RotembergB_correlations_unemploy.csv", replace
restore
...
export delimited using "$output/Table/RotembergC_top5_unemploy.csv", replace
