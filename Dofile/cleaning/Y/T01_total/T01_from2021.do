...
capture mkdir "$interim/ABS/Y/T01/2021"
...
forvalues i = 1(1)547 {
    ...
    * 2021 폴더는 전부 .xlsx라 파일 존재여부만 확인
    ...
    if _rc {
        di as error "SKIP: T01 2021 i=`i' (`i'.`ext' not found/locked)"
        continue
    }
    ...
    * B/C/D=2011, F/G/H=2016, J/K/L=2021 (원본 시트 직접 확인함)
    rename (B C D) (male_2011 female_2011 persons_2011)
    rename (F G H) (male_2016 female_2016 persons_2016)
    rename (J K L) (male_2021 female_2021 persons_2021)
    ...
    save "$interim/ABS/Y/T01/2021/cob`i'.dta", replace
}
**********************************************************************
* PART 2: cob`i'.dta -> long으로 reshape
* 해당파트 (파트3전까지 한번에 돌려야함)
**********************************************************************
cd "$interim/ABS/Y/T01/2021"
...
use "`first'", clear
append using `rest'
save "T01_2021_long.dta", replace
**********************************************************************
* PART 3: 15-64/65+/15+ 연령대 합산 + LGAFINAL21 크로스워크 머지(한 번만) + 최종 저장
**********************************************************************
use "T01_2021_long.dta", clear
...
save "$data/ABS_T01_2021census.dta", replace
