...
* Purpose: T26(Occupation by Sex) -> 지역-연도 패널
*          2006 census (1996, 2001, 2006 년도 정보 포함)
*          T04/T22/T24와 달리 시트 하나("T 26")에 연도가 "세로 행블록"이 아니라
*          T01/T02처럼 "가로 컬럼그룹"으로 나란히 있음 (1996: B/C/D=M/F/Persons,
*          2001: F/G/H, 2006: J/K/L) -> 연령 구분 없음(occupation by SEX만)
*
*          Persons 컬럼(D/H/L)은 스킵, Males/Females만 사용
*          "Inadequately described/Not stated" 행은 스킵
*
*          긁어오는 직업분류(8개, 각각 별도 변수 유지) + Total(그랜드토탈):
*          Managers / Professionals / Technicians and trades workers(b) /
...
forvalues i = 1/`nfiles' {
    ...
    * 2006 폴더는 확장자가 xls/xlsx로 섞여있어서 둘 다 확인
    ...
    if "`ext'"=="" {
        di as error "SKIP: T26 2006 i=`i' (xls/xlsx not found/locked)"
        continue
    }
    ...
    * LGA 라벨 한 줄
    ...
    if _rc {
        di as error "SKIP: T26 2006 i=`i' (`sheet' sheet not found)"
        continue
    }
    keep A
    keep in 2
    ...
    * 본 데이터: 필요한 행만 라벨 기준으로 (Inadequately described/Not stated 제외)
    ...
    keep if A=="Managers" | A=="Professionals" | A=="Technicians and trades workers(b)" | ///
    ...
    * B/C=1996(M/F), F/G=2001(M/F), J/K=2006(M/F) (원본 시트 직접 확인함)
    rename (B C) (male_1996 female_1996)
    rename (F G) (male_2001 female_2001)
    rename (J K) (male_2006 female_2006)
    ...
    save "$interim/ABS/Y/T2635/2006/cob`i'.dta", replace
}
...
forvalues i = 1(1)671 {
    ...
    * 변수명 규칙: {직업분류}_{성별} (T26은 연령 구분이 없어서 연령코드 없음)
    ...
    gen techtrades_m     = male_   if A=="Technicians and trades workers(b)"
    gen techtrades_f     = female_ if A=="Technicians and trades workers(b)"
...
use "`first'", clear
append using `rest'
save "T26_2006_long.dta", replace 
**********************************************************************
* PART 3: LGAFINAL21 크로스워크 머지(한 번만) + 최종 저장
**********************************************************************
use "T26_2006_long.dta", clear
...
save "$data/ABS_T26_2006census.dta", replace
