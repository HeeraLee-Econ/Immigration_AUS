...
* Purpose: T22(Non-School Qualification: Level of Education by Age by Sex) -> 지역-연도 패널
*          2006 census (1996, 2001, 2006 년도 정보 포함)
*          T 22a(Males) + T 22b(Females) 시트를 사용 (T 22c=Persons는 스킵)
*          T04와 마찬가지로 한 시트 안에 연도가 "세로 행블록"으로 3번 쌓여있음
*          (마커: "1996 CENSUS - MALES" 등) -> fill-down으로 year 부여
*
*          컬럼: A=교육수준 라벨, B=15-19, C=20-24, D=25-34, ..., K=Total(그 교육수준의 전연령 합)
*          긁어오는 교육수준(4개, 각각 별도 변수 유지): Postgraduate Degree / Graduate Diploma and
*          Graduate Certificate / Bachelor Degree / Advanced Diploma and Diploma (= "diploma 이상")
...
local nfiles = 671
**********************************************************************
* PART 1: 671개 LGA 원자료 -> T22a(Males)+T22b(Females) 합쳐서 cob`i'.dta 저장
...
forvalues i = 1/`nfiles' {
    ...
    * 2006 폴더는 확장자가 xls/xlsx로 섞여있어서 둘 다 확인
    ...
    if "`ext'"=="" {
        di as error "SKIP: T22 2006 i=`i' (xls/xlsx not found/locked)"
        continue
    }

    * LGA 라벨 한 줄 (T 22a 기준)
    capture noisily import excel using `i'.`ext', sheet("T 22a") clear
    if _rc {
        di as error "SKIP: T22 2006 i=`i' (T 22a sheet not found)"
        continue
    }
    keep A
    keep in 2
    ...
    * ---- T 22a: MALES, 1996+2001+2006 세 연도블록 ----
    import excel using `i'.`ext', sheet("T 22a") clear
    keep A B C D K
    ...
    gen keep_row = 0
    replace keep_row = 1 if inlist(A, "Postgraduate Degree","Graduate Diploma and Graduate Certificate", ///
        "Bachelor Degree","Advanced Diploma and Diploma")
    replace keep_row = 1 if istotal==1 & occurrence_num==2
    keep if keep_row==1

    gen catcode = ""
    replace catcode = "postgrad"   if A=="Postgraduate Degree"
    replace catcode = "graddip"    if A=="Graduate Diploma and Graduate Certificate"
    replace catcode = "bachelor"   if A=="Bachelor Degree"
    replace catcode = "advdip"     if A=="Advanced Diploma and Diploma"
    ...
    * ---- T 22b: FEMALES, 1996+2001+2006 세 연도블록 ----
    import excel using `i'.`ext', sheet("T 22b") clear
    keep A B C D K
    ...
    save "$interim/ABS/Y/T2231/2006/cob`i'.dta", replace
}
...
use "`first'", clear
append using `rest'
save "T22_2006_long.dta", replace
**********************************************************************
* PART 3: LGAFINAL21 크로스워크 머지(한 번만) + 최종 저장
**********************************************************************
use "T22_2006_long.dta", clear
...
save "$data/ABS_T22_2006census.dta", replace
